...
func (iu *ImputationUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := iu.mutation.AmountApply(); ok {
		_spec.SetField(imputation.FieldAmountApply, field.TypeFloat64, value)
	}
	if value, ok := iu.mutation.AddedAmountApply(); ok {
		_spec.AddField(imputation.FieldAmountApply, field.TypeFloat64, value)
	}
	if value, ok := iu.mutation.PaymentAmount(); ok {
		_spec.SetField(imputation.FieldPaymentAmount, field.TypeFloat64, value)
	}
	if value, ok := iu.mutation.AddedPaymentAmount(); ok {
		_spec.AddField(imputation.FieldPaymentAmount, field.TypeFloat64, value)
	}
	if value, ok := iu.mutation.InvoiceAmount(); ok {
		_spec.SetField(imputation.FieldInvoiceAmount, field.TypeFloat64, value)
	}
	if value, ok := iu.mutation.AddedInvoiceAmount(); ok {
		_spec.AddField(imputation.FieldInvoiceAmount, field.TypeFloat64, value)
	}
