...
type InvoiceQuery struct {
	...
	withImputations *ImputationQuery
	withTravelItems *TravelItemQuery
	withFKs         bool
...
// QueryImputations chains the current query on the "imputations" edge.
func (iq *InvoiceQuery) QueryImputations() *ImputationQuery {
	query := (&ImputationClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryTravelItems chains the current query on the "travel_items" edge.
func (iq *InvoiceQuery) QueryTravelItems() *TravelItemQuery {
	query := (&TravelItemClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (iq *InvoiceQuery) Clone() *InvoiceQuery {
	...
	return &InvoiceQuery{
		...
		withImputations: iq.withImputations.Clone(),
		withTravelItems: iq.withTravelItems.Clone(),
...
// WithImputations tells the query-builder to eager-load the nodes that are connected to
// the "imputations" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *InvoiceQuery) WithImputations(opts ...func(*ImputationQuery)) *InvoiceQuery {
	query := (&ImputationClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withImputations = query
	return iq
}

// WithTravelItems tells the query-builder to eager-load the nodes that are connected to
// the "travel_items" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *InvoiceQuery) WithTravelItems(opts ...func(*TravelItemQuery)) *InvoiceQuery {
	query := (&TravelItemClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withTravelItems = query
	return iq
}
...
//		GroupBy(invoice.FieldCreationDate).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Invoice.Query().
//		Select(invoice.FieldCreationDate).
//		Scan(ctx, &v)
...
func (iq *InvoiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invoice, error) {
	var (
		...
		loadedTypes = [3]bool{
			iq.withCustomer != nil,
			iq.withImputations != nil,
			iq.withTravelItems != nil,
		}
	)
	...
	if query := iq.withImputations; query != nil {
		if err := iq.loadImputations(ctx, query, nodes,
			func(n *Invoice) { n.Edges.Imputations = []*Imputation{} },
			func(n *Invoice, e *Imputation) { n.Edges.Imputations = append(n.Edges.Imputations, e) }); err != nil {
			return nil, err
		}
	}
	if query := iq.withTravelItems; query != nil {
		if err := iq.loadTravelItems(ctx, query, nodes,
			func(n *Invoice) { n.Edges.TravelItems = []*TravelItem{} },
			func(n *Invoice, e *TravelItem) { n.Edges.TravelItems = append(n.Edges.TravelItems, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *InvoiceQuery) loadCustomer(ctx context.Context, query *CustomerQuery, nodes []*Invoice, init func(*Invoice), assign func(*Invoice, *Customer)) error {
	...
	for i := range nodes {
		if nodes[i].id_customer == nil {
			continue
		}
		fk := *nodes[i].id_customer
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "id_customer" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (iq *InvoiceQuery) loadImputations(ctx context.Context, query *ImputationQuery, nodes []*Invoice, init func(*Invoice), assign func(*Invoice, *Imputation)) error {
	...
	query.withFKs = true
	query.Where(predicate.Imputation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(invoice.ImputationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.id_invoice
		if fk == nil {
			return fmt.Errorf(`foreign-key "id_invoice" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "id_invoice" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (iq *InvoiceQuery) loadTravelItems(ctx context.Context, query *TravelItemQuery, nodes []*Invoice, init func(*Invoice), assign func(*Invoice, *TravelItem)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Invoice)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.TravelItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(invoice.TravelItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
