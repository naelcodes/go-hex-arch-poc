...
func (pq *PaymentQuery) QueryImputations() *ImputationQuery {
	query := (&ImputationClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PaymentQuery) Clone() *PaymentQuery {
	...
	return &PaymentQuery{
		...
		withImputations: pq.withImputations.Clone(),
...
func (pq *PaymentQuery) WithImputations(opts ...func(*ImputationQuery)) *PaymentQuery {
	query := (&ImputationClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withImputations = query
	return pq
}
...
func (pq *PaymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	...
	if query := pq.withCustomer; query != nil {
		if err := pq.loadCustomer(ctx, query, nodes, nil,
			func(n *Payment, e *Customer) { n.Edges.Customer = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withImputations; query != nil {
		if err := pq.loadImputations(ctx, query, nodes,
			func(n *Payment) { n.Edges.Imputations = []*Imputation{} },
			func(n *Payment, e *Imputation) { n.Edges.Imputations = append(n.Edges.Imputations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PaymentQuery) loadCustomer(ctx context.Context, query *CustomerQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Customer)) error {
...
func (pq *PaymentQuery) loadImputations(ctx context.Context, query *ImputationQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Imputation)) error {
	...
	query.Where(predicate.Imputation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(payment.ImputationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.id_payment_received
		if fk == nil {
			return fmt.Errorf(`foreign-key "id_payment_received" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "id_payment_received" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
