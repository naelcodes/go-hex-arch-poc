...
func (application *Application) CreatePaymentService(paymentDTO *dto.CreatePaymentDTO) (*dto.GetPaymentDTO, error) {
	...
	totalRowCount, err := application.paymentRepository.Count()
	if err != nil {
		application.Logger.Error(fmt.Sprintf("payment records Count error: %v", err))
		return nil, err
	}

	application.Logger.Info(fmt.Sprintf("payment records Count: %v", totalRowCount))
	...
	if err != nil {
		application.Logger.Error(fmt.Sprintf("payment save error: %v", err))
		return nil, err
	}

	application.Logger.Info(fmt.Sprintf("payment saved: %v", savedPaymentDTO))
