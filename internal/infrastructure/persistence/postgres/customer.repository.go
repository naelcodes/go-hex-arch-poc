...
func (repo *CustomerRepository) GetById(id types.EID) (*dto.GetCustomerDTO, error) {
	...
	if err != nil {
		...
		repo.Logger.Error(fmt.Sprintf("[CustomerRepository - GetById] Error getting customer record: %v", err))
	...
	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - GetById] Customer record: %v", customer))

	customerDTO := CustomerModelToDTO(customer)

	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - GetById] Customer DTO: %v", customerDTO))
...
func (repo *CustomerRepository) GetAll(queryParams *types.GetQueryParams) ([]*dto.GetCustomerDTO, error) {
	...
	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - GetAll] QueryParams: %v", queryParams))
	...
	if queryParams != nil {
		...
		if queryParams.Fields != nil {
			...
			repo.Logger.Info(fmt.Sprintf("[CustomerRepository - GetAll] Fields: %v", fields))
		...
		repo.Logger.Info(fmt.Sprintf("[CustomerRepository - GetAll] PageNumber: %v", *queryParams.PageNumber))
		repo.Logger.Info(fmt.Sprintf("[CustomerRepository - GetAll] PageSize: %v", *queryParams.PageSize))
	...
	if err != nil {
		...
		repo.Logger.Error(fmt.Sprintf("[CustomerRepository - GetAll] Error getting customers records: %v", err))
		return nil, CustomErrors.RepositoryError(fmt.Errorf("error getting customers records: %v", err))
	}

	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - GetAll] Customers records: %v", customers))
	customerDTOList := CustomerModelListToDTOList(customers)
	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - GetAll] Customers DTO: %v", customerDTOList))
...
func (repo *CustomerRepository) Count() (*int, error) {
	...
	repo.Logger.Info("[CustomerRepository - Count] counting customers")

	totalRowCount, err := repo.Database.Customer.Query().Where(customer.TagEQ(customer.Tag3)).Count(repo.Context)

	if err != nil {
		repo.Logger.Error(fmt.Sprintf("[CustomerRepository - Count] Error counting customers: %v", err))
		return nil, CustomErrors.RepositoryError(fmt.Errorf("error counting customers: %v", err))
	}

	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - Count] Total row count: %v", totalRowCount))
...
func (repo *CustomerRepository) Save(customer *customerDomain.Customer) (*dto.GetCustomerDTO, error) {
	...
	if err != nil {
		...
		repo.Logger.Error(fmt.Sprintf("[CustomerRepository - Save] Error saving customer record: %v", err))
		return nil, CustomErrors.RepositoryError(fmt.Errorf("error saving customer record: %v", err))
	}

	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - Save] Customer record: %v", customerRecord))
	customerDTO := CustomerModelToDTO(customerRecord)
	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - Save] Customer DTO: %v", customerDTO))

	return customerDTO, nil
}
func (repo *CustomerRepository) Update(customer *customerDomain.Customer) error {

	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - Update] Customer Entity: %v", customer))
	...
	if err != nil {
		...
		repo.Logger.Error(fmt.Sprintf("[CustomerRepository - Update] Error updating customer record: %v", err))
		return CustomErrors.RepositoryError(fmt.Errorf("error updating customer record: %v", err))
	}

	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - Update] Updated customer record: %v", updatedCustomer))

	return nil
}
func (repo *CustomerRepository) Delete(id types.EID) error {

	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - Delete] Id: %v", id))

	deletedCount, err := repo.Database.Customer.Delete().Where(customer.IDEQ(int(id))).Exec(repo.Context)

	if err != nil {
		repo.Logger.Error(fmt.Sprintf("[CustomerRepository - Delete] Error deleting customer record: %v", err))
		return CustomErrors.RepositoryError(fmt.Errorf("error deleting customer record: %v", err))
	}

	repo.Logger.Info(fmt.Sprintf("[CustomerRepository - Delete] Deleted customer record count: %v", deletedCount))

	if deletedCount == 0 {

		repo.Logger.Error(fmt.Sprintf("[CustomerRepository - Delete] Customer record  with id %v not found", id))
